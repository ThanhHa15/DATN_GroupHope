...
public interface OrderRepository extends JpaRepository<Order, Long>,
       ...
       // OrderRepository - Sửa query tổng doanh thu
       @Query(value = """
                         SELECT
                             COALESCE(SUM(o.total_price - COALESCE(o.discount_amount, 0)), 0) AS totalRevenue,
                             COALESCE(COUNT(o.id), 0) AS totalOrders,
                             COALESCE(SUM(CASE WHEN o.order_date >= DATEADD(DAY, -30, GETDATE()) THEN o.total_price - COALESCE(o.discount_amount, 0) ELSE 0 END), 0) AS lastMonthRevenue,
                             COALESCE(COUNT(CASE WHEN o.order_date >= DATEADD(DAY, -30, GETDATE()) THEN o.id END), 0) AS lastMonthOrders
                         FROM orders o
                         WHERE o.payment_status = N'Đã thanh toán'
                     """, nativeQuery = true)
       Object[] getTotalRevenueAndOrders();
}
