...
public interface ProductVariantRepository extends JpaRepository<ProductVariant, Integer> {
        ...
        // Thêm vào ProductVariantRepository

        @Query("SELECT pv FROM ProductVariant pv WHERE pv.discount > 0 AND pv.discountEnd >= CURRENT_DATE")
        Page<ProductVariant> findDiscountedVariants(Pageable pageable);

        @Query("SELECT pv FROM ProductVariant pv WHERE pv.discount > 0 AND pv.discountEnd >= CURRENT_DATE " +
                        "AND LOWER(pv.product.productName) LIKE LOWER(CONCAT('%', :keyword, '%'))")
        Page<ProductVariant> findDiscountedVariantsByKeyword(@Param("keyword") String keyword, Pageable pageable);

        @Query("SELECT pv FROM ProductVariant pv WHERE pv.discount > 0 AND pv.discountEnd >= CURRENT_DATE " +
                        "AND pv.product.category.categoryID = :categoryId")
        Page<ProductVariant> findDiscountedVariantsByCategoryId(@Param("categoryId") Integer categoryId,
                        Pageable pageable);

        @Query("SELECT pv FROM ProductVariant pv WHERE pv.discount > 0 AND pv.discountEnd >= CURRENT_DATE " +
                        "AND (:keyword IS NULL OR LOWER(pv.product.productName) LIKE LOWER(CONCAT('%', :keyword, '%'))) "
                        +
                        "AND (:categoryId IS NULL OR pv.product.category.categoryID = :categoryId)")
        Page<ProductVariant> findDiscountedVariantsWithFilters(@Param("keyword") String keyword,
                        @Param("categoryId") Integer categoryId,
                        Pageable pageable);

        /**
         * Lọc variants với nhiều tiêu chí kết hợp
         */
        @Query("SELECT pv FROM ProductVariant pv WHERE " +
                        "(:categoryId IS NULL OR :categoryId = 0 OR pv.product.category.categoryID = :categoryId) AND "
                        +
                        "(:status IS NULL OR :status = '' OR " +
                        "  (:status = 'con' AND pv.quantityInStock > 0) OR " +
                        "  (:status = 'het' AND pv.quantityInStock <= 0)) AND " +
                        "(:keyword IS NULL OR :keyword = '' OR " +
                        "  LOWER(pv.product.productName) LIKE LOWER(CONCAT('%', :keyword, '%')) OR " +
                        "  LOWER(pv.color) LIKE LOWER(CONCAT('%', :keyword, '%')) OR " +
                        "  LOWER(pv.storage) LIKE LOWER(CONCAT('%', :keyword, '%')))")
        Page<ProductVariant> findVariantsWithMultipleFilters(@Param("categoryId") Integer categoryId,
                        @Param("status") String status,
                        @Param("keyword") String keyword,
                        Pageable pageable);

        /**
         * Tìm kiếm variants theo tên với các bộ lọc bổ sung
         */
        @Query("SELECT pv FROM ProductVariant pv WHERE " +
                        "(:keyword IS NULL OR :keyword = '' OR " +
                        "  LOWER(pv.product.productName) LIKE LOWER(CONCAT('%', :keyword, '%')) OR " +
                        "  LOWER(pv.color) LIKE LOWER(CONCAT('%', :keyword, '%')) OR " +
                        "  LOWER(pv.storage) LIKE LOWER(CONCAT('%', :keyword, '%'))) AND " +
                        "(:categoryId IS NULL OR :categoryId = 0 OR pv.product.category.categoryID = :categoryId) AND "
                        +
                        "(:status IS NULL OR :status = '' OR " +
                        "  (:status = 'con' AND pv.quantityInStock > 0) OR " +
                        "  (:status = 'het' AND pv.quantityInStock <= 0))")
        Page<ProductVariant> searchVariantsByNameWithFilters(@Param("keyword") String keyword,
                        @Param("categoryId") Integer categoryId,
                        @Param("status") String status,
                        Pageable pageable);
}
